...
function convertLatin1FileToUtf8(file) {
  ...
  const after = hasLatin1Orphans(fs.readFileSync(file));
  console.log('FIXED', file, 'orphans before', orphans.length, 'after', after.length);
  return true;
}
...
let n = 0;
files.forEach((f) => {
  if (convertLatin1FileToUtf8(path.join(root, f))) n++;
});
